...
class DinkNet34(nn.Module):
    def __init__(self, num_classes=2, num_channels=3):
        ...
        filters = [64, 128, 256, 512]
        resnet = models.resnet34(pretrained=True)
        #self.prevconv = nn.Conv2d(1, 3, kernel_size=1, stride=1, padding=0, bias=False)
        #self.firstconv = resnet.conv1
...
class U_Net34(nn.Module):
    def __init__(self, num_classes=1, num_channels=3):
        super(U_Net34, self).__init__()
...
class DinkNet50(nn.Module):
    def __init__(self, num_classes=1, num_channels=1):
        ...
        self.finalrelu1 = nonlinearity
        self.finaldropout1 = nn.Dropout(p=0.5)  # 添加dropout层
        self.finalconv2 = nn.Conv2d(32, 32, 3)
        self.finalrelu2 = nonlinearity
        self.finaldropout2 = nn.Dropout(p=0.5)  # 添加dropout层
    ...
    def forward(self, x):
        ...
        out = self.finaldeconv1(d1)
        out = self.finalrelu1(out)
        out = self.finaldropout1(out)  # 应用dropout
        out = self.finalconv2(out)
        out = self.finalrelu2(out)
        out = self.finaldropout2(out)  # 应用dropout
        out = self.finalconv3(out)
