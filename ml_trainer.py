...
class MTTrainer:
    ...
    def __init__(self, model_name='DinkNet50', in_channels=1, out_channels=1, device=None, verbose=False):
        ...
        # 训练参数 - 从ParamConfig获取，确保与用户设置一致
        try:
            import ParamConfig
            self.epochs = getattr(ParamConfig, 'Epochs', 200)
            self.batch_size = getattr(ParamConfig, 'BatchSize', 20)
            self.learning_rate = getattr(ParamConfig, 'LearnRate', 0.001)
            self.early_stop = getattr(ParamConfig, 'EarlyStop', 20)
            self.display_step = getattr(ParamConfig, 'DisplayStep', 10)
            # save_epoch可能没有对应的ParamConfig参数，使用默认值
            self.save_epoch = 10
        except ImportError:
            # 如果无法导入ParamConfig，使用默认值
            self.epochs = 200
            self.batch_size = 20
    ...
    def _train_in_thread(self):
        ...
        try:
            ...
            # 从ParamConfig获取所有训练参数，确保与用户设置一致
            try:
                import ParamConfig
                # 更新所有训练参数
                correct_epochs = getattr(ParamConfig, 'Epochs', 200)
                correct_batch_size = getattr(ParamConfig, 'BatchSize', 20)
                correct_learning_rate = getattr(ParamConfig, 'LearnRate', 0.001)
                correct_early_stop = getattr(ParamConfig, 'EarlyStop', 20)
                correct_display_step = getattr(ParamConfig, 'DisplayStep', 10)
                correct_models_dir = getattr(ParamConfig, 'ModelsDir', 'models/')
                
                # 只在值不同时更新并记录
                if correct_epochs != self.epochs:
                    self._log(f"更新 epochs 值: {self.epochs} → {correct_epochs}")
                    self.epochs = correct_epochs
                if correct_batch_size != self.batch_size:
                    self._log(f"更新 batch_size 值: {self.batch_size} → {correct_batch_size}")
                    self.batch_size = correct_batch_size
                if correct_learning_rate != self.learning_rate:
                    self._log(f"更新 learning_rate 值: {self.learning_rate} → {correct_learning_rate}")
                    self.learning_rate = correct_learning_rate
                if correct_early_stop != self.early_stop:
                    self._log(f"更新 early_stop 值: {self.early_stop} → {correct_early_stop}")
                    self.early_stop = correct_early_stop
                if correct_display_step != self.display_step:
                    self._log(f"更新 display_step 值: {self.display_step} → {correct_display_step}")
                    self.display_step = correct_display_step
                if correct_models_dir != self.models_dir:
                    self._log(f"更新 models_dir 值: {self.models_dir} → {correct_models_dir}")
                    self.models_dir = correct_models_dir
                    # 确保新的模型目录存在
                    if not os.path.exists(self.models_dir):
                        os.makedirs(self.models_dir)
                        self._log(f"已创建新的模型保存目录: {self.models_dir}")
            except ImportError as e:
                self._log(f"无法导入ParamConfig: {str(e)}")
                ...
                if ready_to_read:
                    ...
                    if line:
                        ...
                        def update_gui():
                            ...
                            if "Epoch:" in line and "finished, Loss:" in line and "Loss1:" in line and "Time:" in line:
                                try:
                                    # 这是MT_train.py中专门为解析设计的格式: "Epoch: X finished, Loss: Y, Loss1: Z, Time: T"
                                    # Loss2已被移除
                                    # 提取epoch信息
